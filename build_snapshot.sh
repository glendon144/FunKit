...
# =============================================================================
# PiKit Snapshot Build Script — Safe, non-recursive version
#
...
echo "📦 Starting PiKit Snapshot Build (safe mode)..."

# Snapshot destination (kept the same default)
SNAPSHOT_DIR="${SNAPSHOT_DIR:-$HOME/pikit-snapshots}"
...
BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pikit_snapshot_build.XXXXXX")"
...
SNAPSHOT_NAME="${SNAPSHOT_NAME:-pikit-$(date +%Y%m%d-%H%M%S)}"
...
# -----------------------------------------------------------------------------
echo "📄 Building standalone PiKit binary..."
