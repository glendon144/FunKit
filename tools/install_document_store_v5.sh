...
# v5: get_document_index() returns dicts with id/title/description; add _tuples alias.
#     Keeps v4 self-healing schema and defensive reads.
...
class DocumentStore:
    ...
    - Legacy-friendly API surface (aliases)
    """
    ...
    def get_document_index(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        for d in docs:
            ...
            # description: short snippet from content (no newlines)
            snippet = (d.get("content") or "").strip().replace("\n", " ")
            if len(snippet) > 120:
                snippet = snippet[:117] + "..."
            out.append({"id": d["id"], "title": title, "description": snippet})
        return out

    # Back-compat for any callers that wanted tuples
