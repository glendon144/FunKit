...
LOCAL = Provider(
    ...
    api_key=os.getenv("PIKIT_OPENAI_API_KEY", "pikit-local"),
...
def _pikit_status(self, msg: str):
    try:
...
def _on_provider_changed(self, event=None):
    try:
        ...
        try:
            ...
            pass
        _pikit_status(self, f"Provider: {p.name} ✓")
    except Exception as e:
        _pikit_status(self, f"Provider switch error: {e}")

def _add_provider_dropdown(self, parent_frame):
    if ttk is None:
        print("[PiKit] ttk not available; skipping provider dropdown")
        return
    try:
        ...
        print("[PiKit] Failed to create provider dropdown:", e)
...
try:
    ...
    for _cls_name in ("DemoKitGUI","DemokitGUI","App"):
        if _cls_name in glb and inspect.isclass(glb[_cls_name]):
            ...
            if not hasattr(cls, "_pikit_status"):
                setattr(cls, "_pikit_status", _pikit_status)
    ...
    print("[PiKit] Provider dropdown hook warning:", _e)
...
Environment overrides you can set:
  ...
  export PIKIT_OPENAI_API_KEY=pikit-local
