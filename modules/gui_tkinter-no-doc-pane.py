...
class DemoKitGUI(tk.Tk):
    ...
    def _build_main_pane(self):
        ...
        # Left column = toolbar (fixed), right column = content (expands)
        ...
        self.toolbar.grid(row=0, column=0, sticky="ns", padx=(0, 6))
        buttons = [
            ("ASK", self._on_ask),
            ("SEARCH", self._on_search_clicked),
    ...
    def _on_ask(self):
        try:
            ...
            sel = ""
        if not sel.strip():
            messagebox.showinfo("ASK", "Select some text in the document first.")
            return
    ...
    def _on_ai_success(self, new_doc_id: int):
        messagebox.showinfo("ASK", f"Created new document {new_doc_id}")
    ...
    def _install_search_ui(self):
        ...
        self._search_entry = ttk.Entry(
            self.toolbar, textvariable=self._search_var, width=28
        )
    ...
    def _open_doc_id(self, doc_id: int):
        ...
        def render_document(self, doc_id):
            self.current_doc_id = doc_id
            text = self.store.get_text(doc_id)  # whatever you already use
            self.text.configure(state="normal")
            self.text.delete("1.0", "end")
            self.text.insert("1.0", text)
        # Re-run your link parser on the same text widget
        if hasattr(self, "hypertext"):
            self.hypertext.parse_links(self.text)
            self.text.see("1.0")
