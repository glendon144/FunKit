...
class DemoKitGUI(tk.Tk):
    ...
    def __init__(self, doc_store, processor):
        ...
        self.title("Engelbart Journal – DemoKit — Phase 1 OPML — Phase 2 OPML (Batch) — Phase 2.1 OPML (Batch + Fallback)")
    ...
    def _build_main_pane(self):
        ...
        acts = [
            ...
            ("Batch: Convert to OPML", self._batch_convert_selected_to_opml),
]
    ...
    def _batch_convert_selected_to_opml(self):
        ...
        for did in ids:
            try:
                ...
                if isinstance(body, (bytes, bytearray)):
                    raise RuntimeError("Binary document")
                ...
                # Prepend a markdown link to the OPML version at the top of the original
                header = f"[OPML for {title or 'Document'}](doc:{new_id})\\n\\n"
                new_body = header + (body or "")
                self.doc_store.update_document(did, new_body)
        ...
        self._refresh_sidebar()
        try:
            if getattr(self, "current_doc_id", None) in ids:
        ...
        if bad:
            msg = "; ".join(f"#{d}: {err}" for d, err in bad[:5])
            extra = " (showing first 5)" if len(bad) > 5 else ""
            messagebox.showwarning("Batch OPML", f"{ok} converted, {len(bad)} failed{extra}: {msg}")
        else:
            messagebox.showinfo("Batch OPML", f"Converted {ok} document(s).")
