...
class DemoKitGUI(tk.Tk):
    ...
    def _build_main_pane(self):
        ...
        # Parent grid: left = toolbar, right = content
        self.pane = tk.Frame(self)
        self.pane.grid(row=0, column=1, sticky="nswe", padx=4, pady=4)

        # Lock layout
        self.pane.columnconfigure(0, weight=0)  # toolbar
        self.pane.columnconfigure(1, weight=1)  # content
        self.pane.rowconfigure(0, weight=1)

        # ---------- LEFT: vertical two-column toolbar ----------
        self.toolbar = tk.Frame(self.pane)
        self.toolbar.grid(row=0, column=0, sticky="ns", padx=(0, 6))

        buttons = [
            ("ASK", getattr(self, "_on_ask", None)),
            ("SEARCH", getattr(self, "_on_search_clicked", None)),
            ("TREE", getattr(self, "on_tree_button", None)),
            ("OPEN OPML", getattr(self, "_open_opml_from_main", None)),
            ("DIR IMPORT", getattr(self, "_import_directory", None)),
            ("EXPORT", getattr(self, "_export_doc", None)),
        ]
        ...
        for label, cmd in buttons:
            if cmd is None:
                cmd = lambda: None
                ttk.Button(self.toolbar, text=label, command=cmd, width=12).grid(
        ...
        # ---------- RIGHT: content frame ----------
        self.content = tk.Frame(self.pane)
        ...
        # Rebuild context menu & search row
        if hasattr(self, "_build_context_menu"):
            self._build_context_menu()
        if hasattr(self, "_install_search_ui"):
            self._install_search_ui()
    ...
    def _on_ask(self):
        ...
        self.processor.query_ai(
            ...
            on_success=lambda new_id: messagebox.showinfo(
                "ASK", f"Created new document {new_id}"
            ),
