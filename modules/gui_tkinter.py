...
dir_importer     = _try_import('modules.directory_import', 'import_text_files_from_directory')
...
class DemoKitGUI(tk.Tk):
    ...
    def _save_settings(self) -> None:
        try:
            ...
            data["opml_expand_depth"] = int(getattr(self, "opml_expand_depth", data.get("opml_expand_depth", 2)))
            data["sidebar_width"] = getattr(self, "SIDEBAR_WIDTH", data.get("sidebar_width", 320))
    ...
    def _build_menus(self):
        ...
        viewmenu.add_command(label="Document Tree", command=self.on_tree_button, accelerator="Ctrl+T")
    ...
    def _build_main_pane(self):
        ...
        # simple toolbar row (plugins can add buttons here)
        self.toolbar = tk.Frame(pane)
        self.toolbar.grid(row=1, column=0, sticky="we", pady=(6, 0))
        ...
        for i, (label, cmd) in enumerate([
            ...
            ("EXPORT", self._export_doc),
        ]):
            ttk.Button(self.toolbar, text=label, command=cmd).grid(row=0, column=i, padx=(0, 6))

        # image preview holder
        self.img_label = tk.Label(pane)
        self.img_label.grid_forget()  # only shown when needed
    ...
    def _install_search_ui(self):
        ...
        ttk.Label(self.toolbar, text="Search:").grid(row=0, column=100, padx=(12, 2))
        self._search_entry = ttk.Entry(self.toolbar, textvariable=self._search_var, width=28)
        self._search_entry.grid(row=0, column=101, padx=(0, 6))
        self._search_entry.insert(0, "title or text…")
        self._search_entry.bind("<FocusIn>", lambda e: self._clear_placeholder())
    ...
    def refresh_index(self):
        ...
        try:
            rows = self.doc_store.get_document_index()  # expected: iterable of dicts {id,title,description}
        except Exception:
            rows = []
        for row in rows:
    ...
    def filter_index(self, query: str):
        ...
        self.sidebar.delete(*self.sidebar.get_children())
        try:
            matches = self.doc_store.search_docs(query)  # expected rows: (id, title) or dicts
        except Exception:
            ql = query.lower()
            base = getattr(self.doc_store, "get_document_index", lambda: [])()
            matches = [(d["id"], d.get("title", "")) for d in base if isinstance(d, dict) and (d.get("title","")+d.get("description","")).lower().find(ql) >= 0]
        for row in matches:
            if isinstance(row, dict):
        ...
        class _Repo:
            ...
            def get_doc(self, doc_id):
                ...
                if isinstance(d, dict):
                    return type("Node",(object,),{"id":d.get("id"),"title":d.get("title") or "(untitled)","parent_id":None})()
                return type("Node",(object,),{"id":d[0], "title": (d[1] if len(d)>1 else "(untitled)"), "parent_id":None})()
            def get_children(self, parent_id):
                if parent_id is None:
                    ...
                    for r in self.ds.get_document_index():
                        ...
                        body = d.get("body") if isinstance(d,dict) else (d[2] if len(d)>2 else "")
                        if not isinstance(body,str): continue
                        refd.update(int(m) for m in re.findall(r"\(doc:(\d+)\)", body))
                    roots=[self.get_doc(i) for i in ids if i not in refd] or [self.get_doc(i) for i in ids]
                    return [n for n in roots if n]
                d=self.ds.get_document(parent_id)
                body = d.get("body") if isinstance(d,dict) else (d[2] if len(d)>2 else "")
                ids=[int(m) for m in re.findall(r"\(doc:(\d+)\)", body)] if isinstance(body,str) else []
                return [self.get_doc(i) for i in ids if self.get_doc(i)]
        win = tree_view_mod(self, repo=_Repo(self.doc_store), on_open_doc=self._open_doc_id, root_doc_id=self.current_doc_id)
    ...
    def _import_doc(self):
        path = filedialog.askopenfilename(title="Import", filetypes=[("Text", "*.txt *.md *.html *.opml *.xml"), ("All files", "*.*")])
        if not path:
            return
        try:
    ...
    def _export_doc(self):
        ...
        path = filedialog.asksaveasfilename(title="Export Document", defaultextension=ext, initialfile=f"{safe}{ext}")
        if not path:
            return
        try:
    ...
    def _import_directory(self):
        if not dir_importer:
            messagebox.showwarning("Directory Import", "Directory import module not available.")
            return
        ...
        try:
            ...
            return
        messagebox.showinfo("Directory Import", f"Imported {imported} file(s), skipped {skipped}.")
        self.refresh_index()
    ...
    def _render_document(self, doc):
        ...
        try:
            if hypertext_parser:
                hypertext_parser.parse_links(self.text, body or "", self._on_green_link_click)
        except Exception:
            pass
    ...
    def _ensure_opml_widgets(self):
        ...
        ttk.Checkbutton(tb, text="Show Numbers", variable=self._opml_show_nums, command=self._opml_update_numbering).pack(side=tk.LEFT, padx=6)
    ...
    def _render_opml_from_string(self, s: str):
        ...
        def insert_elem(parent_iid, elem):
            text = elem.attrib.get("text") or elem.attrib.get("title") or (elem.text.strip() if elem.text else "") or "[No Text]"
    ...
    def _set_opml_expand_depth(self):
        try:
            val = simpledialog.askinteger("OPML Expand Depth", "How deep to expand OPML trees (1–6)?", parent=self, minvalue=1, maxvalue=6, initialvalue=self.opml_expand_depth)
            if val:
