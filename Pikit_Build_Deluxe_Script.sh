#!/bin/bash
# =============================================================================
# PiKit Snapshot Build Script — Heavily Commented Edition
#
# Document summary
# - Purpose:
#   Creates a timestamped, self-contained snapshot build (PyInstaller one-file)
#   of the PiKit project and deposits the binary in $HOME/pikit-snapshots.
#
...
#
# - Primary outputs:
#   $HOME/pikit-snapshots/pikit_snapshot_YYYY-MM-DD_HH-MM-SS.bin
#
# - Use cases:
...
#   • The script places the build directory (pikit_snapshot_build) inside the project root.
...
echo "📦 Starting PiKit Snapshot Build..."
...
SNAPSHOT_NAME="pikit_snapshot_$(date +%Y-%m-%d_%H-%M-%S)"

# SNAPSHOT_DIR is where the final binary will be moved for safekeeping.
SNAPSHOT_DIR="$HOME/pikit-snapshots"
...
BUILD_DIR="pikit_snapshot_build"
...
echo "📄 Building standalone PiKit binary with full storage included..."
pyinstaller main.py --onefile --clean --name "$SNAPSHOT_NAME"
